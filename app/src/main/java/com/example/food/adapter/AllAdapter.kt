...
class AllAdapter(val context: Context,val list: ArrayList<Rest>):RecyclerView.Adapter<AllAdapter.AllViewHolder>() {
    ...
    override fun onBindViewHolder(holder: AllViewHolder, position: Int) {
        ...
        holder.layout.setOnClickListener {
            ...
            intent.putExtra("id",res.id.toInt())
            intent.putExtra("name",res.name)
            context.startActivity(intent)
        }

        val on=RestData(res.id,res.name,res.rating,res.cost_for_one,res.image_url)
if(DBAsynctask(context,on,1).execute().get()){
    ...
    holder.image.setImageResource(R.drawable.ic_c_fav)
        ...
        holder.image.setOnClickListener {
            rest=RestData(res.id,res.name,res.rating,res.cost_for_one,res.image_url)
            if(DBAsynctask(context,rest,1).execute().get()){
                DBAsynctask(context,rest,3).execute().get()
                holder.image.setImageResource(R.drawable.ic_c_fav)
            }
            else{
              if(DBAsynctask(context,rest,2).execute().get()) {
                  holder.image.setImageResource(R.drawable.ic_color_fav)
              }
                else{
